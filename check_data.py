import h5py
import numpy as np
from collections import Counter

DATA_PATH = "data/ASCAD.h5"
TARGET_BYTE = 2

SBOX = np.array([
    
])

print("Opening ASCAD file...")
with h5py.File(DATA_PATH, 'r') as f:

    # 1. What's in the file
    print(f"\n--- File Structure ---")
    print(f"Top level keys: {list(f.keys())}")
    print(f"Profiling keys: {list(f['Profiling_traces'].keys())}")

    # 2. Load everything
    X_train = f['Profiling_traces/traces'][:]
    Y_train = f['Profiling_traces/labels'][:]
    metadata = f['Profiling_traces/metadata'][:]

    # 3. Label basics
    print(f"\n--- Label Basics ---")
    print(f"Y_train dtype:        {Y_train.dtype}")
    print(f"Y_train min/max:      {Y_train.min()} / {Y_train.max()}")
    print(f"Y_train first 20:     {Y_train[:20]}")

    # 4. Label distribution
    counts = Counter(Y_train.tolist())
    print(f"\n--- Label Distribution ---")
    print(f"Expected per class:   ~{50000//256} (balanced)")
    print(f"Most common labels:   {counts.most_common(5)}")
    print(f"Least common labels:  {counts.most_common()[-5:]}")

    # 5. Metadata structure
    print(f"\n--- Metadata ---")
    print(f"Metadata dtype:       {metadata.dtype}")
    print(f"Metadata fields:      {metadata.dtype.names}")
    print(f"First entry:          {metadata[0]}")

    # 6. Label verification
    print(f"\n--- Label Verification (first 5 samples) ---")
    for i in range(5):
        pt    = metadata[i]['plaintext'][TARGET_BYTE]
        key   = metadata[i]['key'][TARGET_BYTE]
        xor   = pt ^ key
        expected = SBOX[xor]
        actual   = Y_train[i]
        match = "✓" if expected == actual else "✗ MISMATCH"
        print(f"  Sample {i}: pt={pt:3d} key={key:3d} "
              f"xor={xor:3d} sbox={expected:3d} "
              f"label={actual:3d}  {match}")

    # 7. Check normalization
    print(f"\n--- Normalization Check ---")
    print(f"X_train mean: {X_train.mean():.4f}  (ideal: ~0)")
    print(f"X_train std:  {X_train.std():.4f}   (ideal: ~1)")
    print(f"Is normalized: {'YES' if abs(X_train.mean()) < 1 else 'NO — raw values'}")

print("\nDone.")
